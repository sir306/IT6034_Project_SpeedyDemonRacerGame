...
func _ready():
	$GameOverPopUp.current_level = "2"
	$VictoryPopup.current_level = "2"
...
func reduce_shield_tunnel(collapse):
	if collapse:
		$VehicleBody/PowerDownAudio.play(0)
...
func repair_shield_tunnel(repair):
	if repair:
		$VehicleBody/PowerUpAudio.play(0)
		var tunnels = get_tree().get_nodes_in_group("tunnel")
		for tunnel in tunnels:
			tunnel.shrink = false
			tunnel.grow = true
...
func victory():
	SaveGame.saved_data["Current_Level"] = "3"
	SaveGame.save_game()
	$VictoryPopup.show()
	get_tree().paused = true
